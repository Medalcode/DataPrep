...
from config.settings import CLEANING_CONFIG, PIPELINE_VERSION, VALIDATION_CONFIG
...
if df_raw is None:
    st.info(
        "👈 **Sube un archivo CSV/Excel** desde el menú lateral, o activa el dataset de demostración."
    )
    st.markdown("""
...
with tab1:
    ...
    pairs = [
        ...
        (
            "Filas (después)",
            after_report.total_rows,
            f"−{before_report.total_rows - after_report.total_rows}",
            "delta-ok",
        ),
        (
            "Duplicados",
            before_report.duplicate_rows,
        ...
        (
            "Alertas",
            len(before_report.alerts),
            "",
            "delta-bad" if before_report.alerts else "delta-ok",
        ),
    ]
    ...
    if before_report.alerts:
        ...
        st.markdown(
            '<div class="alert-ok">✅ No se detectaron alertas</div>', unsafe_allow_html=True
        )
        ...
        col_data.append(
            {
                ...
                "Estado": "✅ OK"
                if before_report.null_pcts.get(col, 0) == 0
...
with tab3:
    st.markdown(
        '<div class="section-title">📉 Nulos por Columna (Antes vs Después)</div>',
        unsafe_allow_html=True,
    )

    null_before = pd.Series(before_report.null_counts, name="Antes")
    null_after = pd.Series(
        {c: after_report.null_counts.get(c, 0) for c in before_report.null_counts}, name="Después"
    )
    ...
    if "total" in df_final.columns:
        st.markdown(
            '<div class="section-title">💰 Distribución de Ventas Totales</div>',
            unsafe_allow_html=True,
        )
    ...
    if "categoria_venta" in df_final.columns:
        ...
        with col_a:
            st.markdown(
                '<div class="section-title">🏷️ Categorías de Venta</div>', unsafe_allow_html=True
            )
    ...
    if region_col and "total" in df_final.columns:
        with col_b:
            st.markdown(
                '<div class="section-title">🗺️ Ventas por Región</div>', unsafe_allow_html=True
            )
